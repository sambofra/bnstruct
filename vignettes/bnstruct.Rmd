...
Bayesian Networks (@pearl1988probabilistic) are a powerful tool for
...
values is referred to as the *raw dataset*. In section [sec:imputation]
...
(@efron1994introduction), and provide it to generate samples of a
...
we call *weighted partially DAG*, which is an adjacency matrix whose
...
`bnstruct` requires R $\geq 2.10$, and depends on
...
network, @lauritzen1988local) and one with missing values (the `Child`
network, @spiegelhalter1993bayesian), in the `extdata` subfolder; the
...
(@efron1994introduction), i.e. random samples with replacement of the
...
Network learning {#sec:networklearning}
----------------
...
performed using bootstrap – more on this later).
...
@silander2012simple), that performs a complete evaluation of the search
...
heuristic (`mmhc`, see @tsamardinos2006max), that performs a statistical
...
algorithm (@friedman1997learning [@friedman1998bayesian]), for learning
...
1. a `BN` object (with a structure);
2. a `matrix` containing the adjacency matrix representing the structure of a network;
3. the string `random.chain` for starting from a randomly sampled chain-like network.
...
absence of an edge in the structure (@friedman1999data). This can be
...
(@dempster1977maximum), which learns the parameters of a network from a
