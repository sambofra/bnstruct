...
(guessing the missing values, by looking at the data){\color{red}, or the modeling of
...
{\color{red} If the dataset contains observations a system in consecutive instants,
...
where each time frame can be associated to a layer. {\color{red}How to learn a Dynamic Bayesian Network is explained in Section \ref{sec:dbns},
while a}n example of learning a Naive Bayes network is given in Section \ref{sec:naivebayes}.
