...
For \Rfunarg{sm}, the available options to limit the number of incoming arcs are \Rfunarg{max.fanin.layers} and \Rfunarg{max.fanin}.
For \Rfunarg{mmhc}, \Rfunarg{hc} and \Rfunarg{mmpc}, \Rfunarg{layer.struct} is used to allow or forbid edges between the layers defined using the \Rfunarg{layering} argument;
\Rfunarg{max.fanin} instead sets the maximum number of incoming arcs (for \Rfunarg{hc}) or the maximum number of neighbouring nodes (for \Rfunarg{mmpc} and \Rfunarg{mmhc}).
