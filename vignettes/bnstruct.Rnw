...
where each time frame can be associated to a layer. An example of learning a Naive Bayes network is given in Section \ref{sec:naivebayes}.
...
\subsubsection{Learning Dynamic Bayesian Networks}
Dynamic Bayesian Networks (DBNs) are Bayesian Networks that represent the evolution in time of a system.
The set of variables $v_1, v_2, \dots v_N$ of a DBN is repeated over a certain number $T$ of time slots,
and variables in time slot $j$ can have descendents in successive time slots $k>j$ but not in the previous time
slots $i<j$. From a structure learning point of view we can therefore consider the task of learning a DBN
...
learning DBNs. It works similarly to \Rmethod{learn.network}, but an additional parameter \Rfunarg{num.time.slots}
can be provided, indicating the number of time slots observed. It is assumed that the dataset is composed by the 
variables $v_{1}^{1}, v_2^1, v_3^1, \dots, v_N^1, v_1^2, v_2^2, \dots, v_N^T$ over $T$ time slots.
...
by specifying the layering for just $N$ variables; such layering will be unfolded over the $T$ time slots.
...
\centerline{\includegraphics[width=0.75\textwidth]{child_crosses_1000.eps}}
...
\centerline{\includegraphics[width=0.75\textwidth]{alarm_crosses_1000.eps}}
...
\centerline{\includegraphics[width=0.75\textwidth]{hepar2_crosses_1000.eps}}
